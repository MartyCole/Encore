...
% coordinates, and the density per triangle is computed. A Connectivity-Level
% Overlap Coefficient is then calculated based on which triangles exceed a 
...
%
% Outputs:
%    COC - The COC: |X & Y| / min(|X|, |Y|);
%    x_dens - The density of X endpoints per triangle
%    y_dens - The density of Y endpoints per triangle
%

function [COC,x_dens,y_dens] = get_overlap_coefficient(X,Y,lh_grid,rh_grid,threshold)
    ...
    COC = sum(X.set .* Y.set) / min(sum(X.set),sum(Y.set));
